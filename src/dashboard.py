...
try:
    ...
    go = None; px = None; PLOTLY_OK = False
...
:root {
  --bg:#F9F8F5;--surface:#FFFFFF;--surface2:#FAF9F6;--border:#E8E3DC;
  --border2:#D5CFC7;--text:#1A1915;--muted:#6B6560;--subtle:#9C9690;
  --accent:#DA7756;--accent-h:#C96644;--accent-bg:#FDF3EF;
  --green:#2D7A4F;--green-bg:#F0FAF4;--green-bdr:#A7F3C9;
  --amber:#B45309;--amber-bg:#FFFBEB;--amber-bdr:#FDE68A;
  --red:#C0392B;--red-bg:#FEF2F2;--red-bdr:#FCA5A5;
  --blue:#2563EB;--blue-bg:#EFF6FF;--blue-bdr:#BFDBFE;
  --purple:#534AB7;--r:10px;--r-sm:6px;
...
.info-box{background:var(--accent-bg);border:1px solid #F5C5B3;border-left:3px solid var(--accent);border-radius:var(--r);padding:12px 16px;margin-bottom:12px;font-size:13px;color:var(--text);line-height:1.6}
...
PLT_BG="#FFFFFF";PLT_AX="#FAF9F6";PLT_TEXT="#1A1915";PLT_GRID="#E8E3DC"
PLT_ACCENT="#DA7756";PLT_GREEN="#2D7A4F";PLT_RED="#C0392B"
PLT_BLUE="#2563EB";PLT_AMBER="#B45309";PLT_PURPLE="#534AB7";PLT_MUTED="#9C9690"

mpl.rcParams.update({
    "figure.facecolor":PLT_BG,"axes.facecolor":PLT_AX,"axes.edgecolor":PLT_GRID,
    "axes.labelcolor":PLT_TEXT,"xtick.color":PLT_TEXT,"ytick.color":PLT_TEXT,
    "text.color":PLT_TEXT,"grid.color":PLT_GRID,"grid.linestyle":"--","grid.alpha":0.6,
    "legend.facecolor":PLT_BG,"legend.edgecolor":PLT_GRID,"legend.labelcolor":PLT_TEXT,"font.size":11,
})
...
def load_csv(filename):
    for path in [
        os.path.join(BASE,filename),os.path.join(BASE,"..","outputs",filename),
        os.path.join(BASE,"data",filename),os.path.join(BASE,"results",filename),
        os.path.join(BASE,"..","src","results",filename),filename,
    ]:
...
def load_json(filename):
    for path in [
        os.path.join(BASE,filename),os.path.join(BASE,"..","outputs",filename),
        os.path.join(BASE,"data",filename),os.path.join(BASE,"results",filename),filename,
    ]:
        if os.path.exists(path):
            with open(path) as f:
                return json.load(f)
...
def normalize_wireless_results(raw,scenario=None):
    ...
    if "metrics" in raw and "parameters" in raw: return raw
    coverage=raw.get("coverage_statistics",{})
    budgets=raw.get("site_link_budgets",[])
    env=(scenario or {}).get("environment",{})
    radius_values=[r.get("coverage_radius_km") for r in budgets if r.get("coverage_radius_km") is not None]
    ...
    normalized=dict(raw)
    normalized["metrics"]={
        "max_radius_km":round(float(max_radius),3) if max_radius else 2.47,
        "service_radius_km":round(float(max_radius),3) if max_radius else 2.47,
        "area_per_site_km2":round(float(area_per_site),3) if area_per_site else None,
...
def normalize_upgrade_plan_df(df):
    if df is None: return None
    out=df.copy()
    col_map={"phase":"Phase","trigger_year":"Trigger","action":"Action","utilisation":"Goal"}
...
#  MODULE IMPORTS
# ══════════════════════════════════════════════════════════════════════════════
TOPOLOGY_LOADED=False;TRAFFIC_MODS=False;SIGNALING_MOD=False
nodes_live,links_live=[],[]
_topology_error=""
G_obj=None;scenario_obj=None
...
try:
    ...
    try:
        ...
        if os.path.exists(_scenario_path):
            ...
            _NODE_COLORS={"CR-1":"#DA7756","CR-2":"#2563EB","BS1":"#534AB7","BS2":"#534AB7","BS3":"#534AB7","BS4":"#534AB7","BS5":"#534AB7"}
            _NODE_ICONS={"CR-1":"CR1","CR-2":"CR2","BS1":"BS","BS2":"BS","BS3":"BS","BS4":"BS","BS5":"BS"}
            for _n in G_obj.nodes():
                _d=G_obj.nodes[_n]
                nodes_live.append({"id":_n,"name":f"{_n} - {_d.get('label','?')}","lat":_d.get("y",0.0)+(-22.45),"lon":_d.get("x",0.0)+(26.5),"location":_d.get("label","?"),"color":_NODE_COLORS.get(_n,"#534AB7"),"icon_label":_NODE_ICONS.get(_n,"BS"),"type":"cr1" if _n=="CR-1" else "cr2" if _n=="CR-2" else "bs","role_label":"Primary Core Router" if _n=="CR-1" else "Backup Core Router" if _n=="CR-2" else "Base Station","tower_height_m":68 if _n=="CR-1" else 66 if _n=="CR-2" else 42,"x_km":_d.get("x",0.0),"y_km":_d.get("y",0.0)})
            _LCOLORS={"primary":"#DA7756","backup":"#2563EB","backbone":"#B45309","microwave_13ghz":"#B45309","lte_priority":"#9C9690","microwave":"#DA7756"}
            _seen=set()
            for _u,_v,_d in G_obj.edges(data=True):
                ...
                if _role=="backup" and "backbone" in (_d.get("link_type","")): _role="backbone"
                if _d.get("link_type","") in ("microwave_13ghz","lte_priority"): _role="backbone"
                links_live.append({"source":_u,"target":_v,"role":_role,"color":_LCOLORS.get(_d.get("link_type",_role),_LCOLORS.get(_role,"#888")),"delay_ms":_d.get("delay_ms",8.0),"capacity_mbps":_d.get("capacity_mbps",100)})
...
try:
    ...
    from teletraffic import (run_teletraffic,compute_signaling_load,signaling_summary,stress_sweep,find_breaking_point,full_dimensioning_table,evaluate_delay_kpis)
...
def _try_compute(scenario):
    ...
    if not (TRAFFIC_MODS and scenario): return
    try:
        if df_load is None: df_load=compute_offered_load(scenario)
        if df_matrix is None: df_matrix=compute_traffic_matrix(scenario)
        if df_dim is None: df_dim=full_dimensioning_table(scenario)
        if df_delay is None: df_delay=evaluate_delay_kpis(scenario)
        if df_stress is None: df_stress=stress_sweep(scenario)
        if df_signal is None: df_signal=compute_signaling_load(scenario)
        if df_signal_summary is None: df_signal_summary=pd.DataFrame([signaling_summary(scenario)])
    except Exception:
        pass
    try:
        if df_util is None or df_plan is None:
            fc=run_forecasting(scenario)
            if df_util is None: df_util=fc["utilisation"]["annual_table"]
            if df_plan is None:
        ...
        try:
            ...
            if os.path.exists(fc_path):
                subprocess.run([sys.executable,fc_path],cwd=BASE,capture_output=True,timeout=30)
                df_util=load_csv("forecasting_utilisation_annual.csv")
                df_plan=normalize_upgrade_plan_df(load_csv("forecasting_upgrade_plan.csv"))
        except Exception:
            pass
...
def get_default_inventory():
    nodes={
        "CR-1":{"id":"CR-1","name":"CR-1 - Palapye District Hospital","lat":-22.5495,"lon":27.1257,"location":"Palapye","color":"#DA7756","icon_label":"CR1","type":"cr1","role_label":"Primary Core Router","tower_height_m":68,"freq_ghz":13.0,"tx_power_dbm":30,"antenna_gain_dbi":32,"beam_width_deg":2},
        "CR-2":{"id":"CR-2","name":"CR-2 - Serowe District Health Office","lat":-22.3880,"lon":26.7108,"location":"Serowe","color":"#2563EB","icon_label":"CR2","type":"cr2","role_label":"Backup Core Router","tower_height_m":66,"freq_ghz":13.0,"tx_power_dbm":30,"antenna_gain_dbi":32,"beam_width_deg":2},
        "BS1":{"id":"BS1","name":"BS1 - Moremi Village","lat":-22.3442,"lon":27.1505,"location":"Moremi","color":"#534AB7","icon_label":"BS","type":"bs","role_label":"Base Station","tower_height_m":42,"freq_ghz":7.0,"tx_power_dbm":43,"antenna_gain_dbi":17,"beam_width_deg":120},
        "BS2":{"id":"BS2","name":"BS2 - Majete","lat":-22.3352,"lon":26.8747,"location":"Majete","color":"#534AB7","icon_label":"BS","type":"bs","role_label":"Base Station","tower_height_m":40,"freq_ghz":7.0,"tx_power_dbm":43,"antenna_gain_dbi":17,"beam_width_deg":120},
        "BS3":{"id":"BS3","name":"BS3 - Topisi","lat":-22.5746,"lon":27.0321,"location":"Topisi","color":"#534AB7","icon_label":"BS","type":"bs","role_label":"Base Station","tower_height_m":39,"freq_ghz":7.0,"tx_power_dbm":43,"antenna_gain_dbi":17,"beam_width_deg":120},
        "BS4":{"id":"BS4","name":"BS4 - Malaka","lat":-22.4780,"lon":26.8222,"location":"Malaka","color":"#534AB7","icon_label":"BS","type":"bs","role_label":"Base Station","tower_height_m":41,"freq_ghz":7.0,"tx_power_dbm":43,"antenna_gain_dbi":17,"beam_width_deg":120},
        "BS5":{"id":"BS5","name":"BS5 - Radisele","lat":-22.4510,"lon":27.3218,"location":"Radisele","color":"#534AB7","icon_label":"BS","type":"bs","role_label":"Base Station","tower_height_m":43,"freq_ghz":7.2,"tx_power_dbm":43,"antenna_gain_dbi":17,"beam_width_deg":120},
    }
    links=[
...
def get_inventory():
    if TOPOLOGY_LOADED and nodes_live:
        ...
        if not links: _,links=get_default_inventory()
        return nodes,links
...
def get_site_traffic_breakdown():
    ...
    if df_matrix is not None and {"site","voice_mbps","video_mbps","telemetry_mbps"}.issubset(df_matrix.columns):
        ...
        for site,row in grouped.iterrows():
            breakdown[site]={"telemetry":float(row["telemetry_mbps"]),"voice":float(row["voice_mbps"]),"video":float(row["video_mbps"])}
...
def simulate_network(nodes_in,links_in,config,faults,quick_failover):
    ...
    for nid,node in nodes.items():
        ...
        node["load_ratio"]=0.0;node["traffic_mbps"]=0.0

    def add_alarm(entity,name,sev,hint,details):
        alarms.append({"time":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"entity":entity,"alarm":name,"severity":sev,"hint":hint,"details":details})

    if quick_failover and "CR-1" in nodes:
        nodes["CR-1"]["status"]="offline"
        add_alarm("CR-1","CORE ROUTER FAILURE","Critical","Primary core unavailable. Verify power, routing daemon, and microwave handoff.","All primary services shifting to CR-2.")
    for bs in failed_bs:
        if bs in nodes:
            nodes[bs]["status"]="offline"
            add_alarm(bs,"BASE STATION FAILURE","Critical","Check site power, radio chain, and antenna alignment.",f"{bs} unavailable. Clinic traffic isolated.")
    if degradation>0:
        sev="Major" if degradation>=0.35 else "Minor"
        add_alarm("Backhaul","BACKHAUL DEGRADATION",sev,"Inspect microwave fade margin, weather impact, and spectrum interference.",f"Primary transport degraded by {degradation*100:.0f}%.")
    if congestion_router!="None" and congestion_pct>0:
        sev="Critical" if congestion_pct>=0.7 else "Major"
        add_alarm(congestion_router,"ROUTER CONGESTION",sev,"Shift flows to alternate router or raise queue bandwidth.",f"{congestion_router} headroom reduced by {congestion_pct*100:.0f}%.")

    for link in links:
        factor={"primary":config["primary_bw_alloc_pct"],"backup":config["backup_bw_alloc_pct"],"backbone":config["backbone_bw_alloc_pct"]}.get(link["role"],100)/100.0
        eff=float(link["capacity_mbps"])*factor
        if link["role"] in {"primary","backbone"}: eff*=(1-degradation)
        if congestion_router!="None" and congestion_router in {link["source"],link["target"]}: eff*=(1-congestion_pct)
        if quick_failover and link["source"]=="CR-1" and link["role"] in {"primary","backbone"}: eff=0.0
        if link["source"] in failed_bs or link["target"] in failed_bs: eff=0.0
    ...
    for bs_id in [n for n in nodes if n.startswith("BS")]:
        ...
        if nodes[bs_id]["status"]=="offline":
            flow_rows.append({"site":bs_id,"demand_mbps":total_demand,"decision":"Unavailable","primary_mbps":0.0,"backup_mbps":0.0,"status":"Critical"})
            continue
        ...
        if primary_cap<=0 and backup_cap>0:
            ...
            bs_share=round(total_demand-ps,3);decision="Load balanced";status="Minor"
        else:
            ps=total_demand
        if primary: primary["assigned_mbps"]+=ps
        if backup: backup["assigned_mbps"]+=bs_share
        nodes[bs_id]["traffic_mbps"]=total_demand
        flow_rows.append({"site":bs_id,"demand_mbps":total_demand,"decision":decision,"primary_mbps":round(ps,3),"backup_mbps":round(bs_share,3),"status":status})

    backbone=link_lookup.get(("CR-1","CR-2","backbone"))
    if backbone: backbone["assigned_mbps"]=round(sum(r["backup_mbps"] for r in flow_rows),3)
    ...
    for link in links:
        ...
        if cap<=0 and demand>0:
            link["status"]="down"
            add_alarm(f"{link['source']}->{link['target']}","TRANSPORT UNAVAILABLE","Critical","No carrying capacity remains.",f"Demand {demand:.2f} Mbps cannot be served.")
        elif cap>0 and demand>cap:
            add_alarm(f"{link['source']}->{link['target']}","CONGESTION ALERT","Critical" if demand>cap*1.15 else "Major","Increase bandwidth, reroute traffic, or add channels.",f"Capacity {cap:.2f} < Demand {demand:.2f} Mbps.")
            link["status"]="overloaded"
        elif cap>0 and demand>cap*0.85:
            add_alarm(f"{link['source']}->{link['target']}","HIGH UTILISATION","Major","Segment nearing saturation.",f"Utilisation {link['load_ratio']*100:.1f}%.")
    ...
    return {"nodes":nodes,"links":links,"alarms":alarms,"flow_rows":flow_rows,"recommendations":recommendations,"traffic_breakdown":breakdown}
...
def append_event(etype,sev,msg,details=""):
    ...
    st.session_state.event_log.append({"timestamp":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"type":etype,"severity":sev,"message":msg,"details":details})
...
def map_tile_config(style_name):
    tiles={
        ...
        "Live Satellite":{"tiles":"https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}","attr":"Esri World Imagery"},
...
def make_antenna_marker(node_id, node, offline=False):
    ...
    beacon_color = "#FF4444" if offline else ("#FFD700" if ntype in ("cr1","cr2") else "#00FF88")
...
def link_flow_specs(link,src,tgt):
    role=link["role"];color=link["color"]
    ...
    if failure_mode:
        if role=="primary": color,weight,opacity,dash="#9C9690",2,0.28,"5,7"
        elif role=="backbone": color,weight,opacity,dash="#9C9690",2,0.22,"4,8"
    if link.get("status")=="overloaded": color,weight,opacity,dash,pulse_color,speed=PLT_RED,5,0.95,"3,5","#FFE1D9",650
    elif link.get("status") in {"degraded","warning"}: color,weight,opacity,pulse_color,speed=PLT_AMBER,max(weight,4),0.92,"#FFF0C2",850
    elif link.get("status")=="down": color,weight,opacity,dash,pulse_color,speed="#9C9690",2,0.3,"2,8","#D6D3D1",2200
    ...
    return {"locations":[[src["lat"],src["lon"]],[tgt["lat"],tgt["lon"]]],"color":color,"weight":weight,"opacity":opacity,"dash":dash,"pulse_color":pulse_color,"speed":speed,"popup_html":popup_html,"tooltip":tooltip}
...
with st.sidebar:
    ...
    section=st.radio("Navigation",[
        ...
        "Antenna Properties",
        "QoS",
        "Routing & Signaling","Signaling Details","Call Termination","Billing Analytics",
        "Stress Test & Fault Management",
        "Forecast","Events"
    ],label_visibility="collapsed")

    with st.expander("Map Options",expanded=True):
        map_tile=st.selectbox("Map style",["OpenStreetMap","CartoDB Light","Live Satellite","Satellite + Labels"],index=0)
    ...
    with st.expander("Failure Simulation",expanded=section=="Stress Test & Fault Management"):
    ...
    if st.button("Run All Modules"):
        import subprocess
        ran=[]
        all_scripts=["traffic.py","teletraffic.py","qos.py","backhaul.py","wireless.py","signaling.py","stress_test.py","forecasting.py","routing.py","propagation.py"]
        for script in all_scripts:
            p=os.path.join(BASE,script)
            if os.path.exists(p):
                subprocess.run([sys.executable,p],cwd=BASE,capture_output=True,timeout=60)
                ran.append(script)
    ...
    if TOPOLOGY_LOADED:
        st.markdown('<span class="badge-pass">topology.py ✓</span>',unsafe_allow_html=True)
    else:
        st.markdown('<span class="badge-warn">topology.py not found</span>',unsafe_allow_html=True)
        if _topology_error:
            st.caption(f"⚠ {_topology_error}")
    if TRAFFIC_MODS:
        st.markdown('<span class="badge-pass">traffic modules ✓</span>',unsafe_allow_html=True)
    else:
        st.markdown('<span class="badge-warn">traffic modules not loaded</span>',unsafe_allow_html=True)

# ─── Build simulation ─────────────────────────────────────────────────────────
BASE_NODES,BASE_LINKS=get_inventory()
SIMULATION=simulate_network(BASE_NODES,BASE_LINKS,st.session_state.nms_config,st.session_state.nms_faults,failure_mode)
...
st.markdown(f'<div class="title-bar"><h1>District Telehealth &amp; Emergency Communication NMS</h1><p>Palapye / Serowe District, Botswana &nbsp;|&nbsp; Dual-homed Star Topology &nbsp;|&nbsp; 7 GHz Microwave &nbsp;|&nbsp; 13 GHz Backbone &nbsp;|&nbsp; 5 Clinic Sites &nbsp;|&nbsp; {failure_badge}</p></div>',unsafe_allow_html=True)

worst_blocking=(f"{df_dim[df_dim['site'].str.contains('BS',na=False)]['achieved_blocking'].max()*100:.2f}%" if df_dim is not None and 'achieved_blocking' in df_dim.columns else "0.62%")
...
if section=="Network Map":
    ...
    with map_col:
        st.markdown('<div class="section-header">Live Network Map — Serowe / Palapye District (OpenStreetMap)</div>',unsafe_allow_html=True)
        ...
        for lnk in LINKS:
            role=lnk["role"]
            if role=="primary" and not show_primary: continue
            if role=="backup" and not show_backup: continue
            if role=="backbone" and not show_backbone: continue
        ...
        for k,node in NODES.items():
            ...
            folium.Marker(
                ...
                icon=folium.DivIcon(
                    html=ant_html,
                    icon_size=(54,icon_h),
                    icon_anchor=(27,icon_h)
                ),
                popup=folium.Popup(f"<b>{node['name']}</b><br>Type: {node.get('role_label','Base Station')}<br>Height: {node.get('tower_height_m','-')} m<br>Status: {'OFFLINE' if is_offline else 'Operational'}<br>Load: {node.get('load_ratio',0)*100:.1f}%",max_width=260),
                tooltip=f"{node['name']} ({'OFFLINE' if is_offline else 'OK'})"
            ).add_to(towers_fg)
...
# ─── INTERACTIVE TOPOLOGY (3D) ────────────────────────────────────────────────
    ...
    if not PLOTLY_OK:
        ...
        for link in SIMULATION["links"]:
            ...
            fig.add_trace(go.Scatter3d(x=[src["lon"],tgt["lon"]],y=[src["lat"],tgt["lat"]],z=[0,0],mode="lines",line={"color":color,"width":3},hoverinfo="skip",showlegend=False))
        for nid,node in SIMULATION["nodes"].items():
            ...
            fig.add_trace(go.Scatter3d(x=[node["lon"]],y=[node["lat"]],z=[float(node.get("tower_height_m",42))],mode="markers+text",text=[nid],textposition="top center",textfont={"color":PLT_TEXT,"size":11},marker={"size":8,"color":color,"line":{"width":1.5,"color":"#ffffff"}},hoverinfo="text",hovertext=[h],showlegend=False))
        fig.update_layout(height=620,margin={"l":0,"r":0,"t":20,"b":0},paper_bgcolor=PLT_BG,plot_bgcolor=PLT_AX,scene={"bgcolor":PLT_AX,"xaxis":{"visible":False},"yaxis":{"visible":False},"zaxis":{"title":"Height (m)","color":PLT_TEXT},"camera":{"eye":{"x":1.75,"y":1.55,"z":1.05}}})
        st.plotly_chart(fig,use_container_width=True,config={"displaylogo":False})

# ─── RF & TRAFFIC ANALYSIS (unified: Traffic + Wireless + Backhaul) ───────────
elif section=="RF & Traffic Analysis":
    st.markdown('<div class="section-header">RF & Traffic Analysis — Unified View</div>',unsafe_allow_html=True)
    rf_tab1,rf_tab2,rf_tab3,rf_tab4=st.tabs(["📶 Offered Load & Matrix","📡 Wireless Coverage","🔗 Backhaul Link Budget","📊 Traffic Summary"])
    ...
    with rf_tab1:
        ...
        if df_load is not None:
            ...
            dscp_map={"voice":'<span class="badge-pass">AF31</span>',"video":'<span class="badge-warn">AF21</span>',"telemetry":'<span class="badge-info">EF</span>'}
            rows="".join(f"<tr><td><b style='color:{PLT_PURPLE}'>{r.site}</b></td><td>{r.service_class.title()}</td><td>{dscp_map.get(r.service_class,'')}</td><td>{r.arrival_rate_per_hour}</td><td>{r.holding_time_s} s</td><td><b>{r.offered_load_erl:.4f}</b></td></tr>" for r in df_load.itertuples())
            ...
            if PLOTLY_OK:
                pivot_load=df_load.groupby(["site","service_class"])["offered_load_erl"].sum().reset_index()
                fig_l=px.bar(pivot_load,x="site",y="offered_load_erl",color="service_class",barmode="stack",color_discrete_map={"voice":PLT_BLUE,"video":PLT_PURPLE,"telemetry":PLT_ACCENT})
                fig_l.update_layout(height=280,paper_bgcolor=PLT_BG,plot_bgcolor=PLT_AX,font={"color":PLT_TEXT},xaxis_title="Base Station",yaxis_title="Erlang",legend={"orientation":"h","y":-0.25},margin={"l":0,"r":0,"t":10,"b":0})
        ...
        if df_matrix is not None:
            ...
            st.markdown('<div class="info-box"><b>Formula:</b> A = λ × h / 3600 &nbsp;|&nbsp; Video dominates at 2–4 Mbps per session vs voice at 0.048 Mbps.</div>',unsafe_allow_html=True)
        else:
            st.warning("Traffic matrix data is currently unavailable.")

    with rf_tab2:
        st.markdown('<div class="section-header">COST 231-Hata Wireless Coverage Analysis</div>',unsafe_allow_html=True)
        if wl:
            ...
            with col1:
                st.markdown(f'<div class="table-wrap"><table class="styled-table"><thead><tr><th>Parameter</th><th>Value</th></tr></thead><tbody><tr><td>Access frequency</td><td>{p["frequency_mhz"]} MHz</td></tr><tr><td>TX power</td><td>{p["tx_power_dbm"]} dBm</td></tr><tr><td>TX antenna gain</td><td>{p.get("tx_gain_dbi","-")} dBi</td></tr><tr><td>RX antenna gain</td><td>{p.get("rx_gain_dbi","-")} dBi</td></tr><tr><td>RX sensitivity</td><td>{p["sensitivity_dbm"]} dBm</td></tr><tr><td>BS height</td><td>{p.get("bs_height_m","-")} m</td></tr><tr><td>Propagation model</td><td>COST 231-Hata</td></tr></tbody></table></div>',unsafe_allow_html=True)
            with col2:
                if df_wireless_thresholds is not None:
                    rows="".join(f"<tr><td>{r.label}</td><td>{r.threshold_dbm} dBm</td><td>{r.baseline_coverage_pct:.2f}%</td><td>{r.improved_coverage_pct:.2f}%</td><td>{r.gain_pct_points:.2f} pts</td></tr>" for r in df_wireless_thresholds.itertuples())
            ...
            if PLOTLY_OK and df_wireless_surface is not None:
                ...
                figw=go.Figure(data=go.Heatmap(z=pivot.values,x=list(pivot.columns),y=list(pivot.index),colorscale="Cividis",colorbar={"title":"dBm"},zmin=-120,zmax=-60))
                figw.update_layout(height=380,margin={"l":0,"r":0,"t":10,"b":0},paper_bgcolor=PLT_BG,plot_bgcolor=PLT_AX,font={"color":PLT_TEXT},xaxis_title="District X (km)",yaxis_title="District Y (km)")
                st.plotly_chart(figw,use_container_width=True)
            st.markdown(f'<div class="info-box"><b>Coverage improvement:</b> {wl.get("improvement_action","Add an infill site or raise antenna height.")}</div>',unsafe_allow_html=True)
        else:
    ...
    with rf_tab3:
        ...
        if bh_results:
            ...
            c1,c2,c3,c4=st.columns(4)
            with c1: st.metric("Total Links",len(bh_results))
            with c2: st.metric("PASS",pass_count)
            with c3: st.metric("FAIL",fail_count)
        ...
        if bh_df_use is not None:
            ...
            if bh_results:
                ...
                ax.bar(links_l,margins,color=[PLT_GREEN if m>=20 else PLT_RED for m in margins],alpha=0.85,zorder=3,width=0.6)
                ...
                fig,ax=plt.subplots(figsize=(11,3.5))
                ax.bar(bh_df_use['source'].astype(str)+'→'+bh_df_use['target'].astype(str),bh_df_use['fade_margin_db'],color=[PLT_GREEN if v>=20 else PLT_RED for v in bh_df_use['fade_margin_db']],alpha=0.85,zorder=3)
                ax.axhline(20,color=PLT_AMBER,linestyle='--',linewidth=2,label='Required (20 dB)',zorder=4)
        ...
        st.markdown('<div class="info-box"><b>Link budget:</b> Rx power = TX power + TX gain − FSPL − misc losses. Rain attenuation: ITU-R P.838-3, Botswana zone H (30 mm/h). Required fade margin ≥ 20 dB.</div>',unsafe_allow_html=True)

    with rf_tab4:
        st.markdown('<div class="section-header">Network-Level Traffic Summary</div>',unsafe_allow_html=True)
        breakdown=SIMULATION["traffic_breakdown"]
        if breakdown:
            bd_rows=[]
            for site,mix in breakdown.items():
                total=sum(mix.values())
                bd_rows.append({"Site":site,"Voice (Mbps)":round(mix.get("voice",0),3),"Video (Mbps)":round(mix.get("video",0),3),"Telemetry (Mbps)":round(mix.get("telemetry",0),4),"Total (Mbps)":round(total,3)})
            bd_df=pd.DataFrame(bd_rows)
            st.dataframe(bd_df,use_container_width=True,hide_index=True)
            if PLOTLY_OK:
                fig_td=go.Figure()
                fig_td.add_trace(go.Bar(x=bd_df["Site"],y=bd_df["Voice (Mbps)"],name="Voice",marker_color=PLT_BLUE))
                fig_td.add_trace(go.Bar(x=bd_df["Site"],y=bd_df["Video (Mbps)"],name="Video",marker_color=PLT_PURPLE))
                fig_td.add_trace(go.Bar(x=bd_df["Site"],y=bd_df["Telemetry (Mbps)"],name="Telemetry",marker_color=PLT_ACCENT))
                fig_td.update_layout(barmode="stack",height=300,paper_bgcolor=PLT_BG,plot_bgcolor=PLT_AX,font={"color":PLT_TEXT},xaxis_title="Base Station",yaxis_title="Traffic (Mbps)",legend={"orientation":"h","y":-0.25},margin={"l":0,"r":0,"t":10,"b":0})
...
elif section=="Antenna Properties":
    st.markdown('<div class="section-header">Antenna Properties & Dimensioning</div>',unsafe_allow_html=True)
    ant_tab1,ant_tab2,ant_tab3=st.tabs(["🗼 Site Antenna Specs","📐 Erlang B Dimensioning","⏱ Delay KPIs"])

    with ant_tab1:
        st.markdown('<div class="section-header">Per-Site Antenna Specifications</div>',unsafe_allow_html=True)
        NODES,_=get_inventory()
        node_list=list(NODES.values())
        col_a,col_b=st.columns([1.2,1],gap="large")
        with col_a:
            for node in node_list:
                nid=node["id"];ntype=node.get("type","bs")
                c=node["color"]
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:10px;font-size:12px">
    ...
    <div><span style="color:#9C9690;display:block">Type</span><b>{'Microwave PTP' if is_cr else 'Sector 120°'}</b></div>
        ...
        with col_b:
            ...
            if PLOTLY_OK:
                ...
                fig_ant=go.Figure()
                fig_ant.add_trace(go.Scatterpolar(r=r_vals,theta=theta,mode="lines",line={"color":PLT_PURPLE,"width":2},fill="toself",fillcolor=f"rgba(83,74,183,0.10)",name="BS Sector"))
                # PTP dish pattern
                r_ptp=[sector_gain(t,bw=2,peak_gain=32) for t in theta]
                fig_ant.add_trace(go.Scatterpolar(r=r_ptp,theta=theta,mode="lines",line={"color":PLT_ACCENT,"width":2},name="CR Microwave (PTP)"))
                fig_ant.update_layout(
                    polar={"radialaxis":{"visible":True,"range":[-5,35],"tickfont":{"size":10}},"angularaxis":{"tickfont":{"size":10}}},
                    height=380,paper_bgcolor=PLT_BG,font={"color":PLT_TEXT},
                    legend={"orientation":"h","y":-0.1},margin={"l":10,"r":10,"t":20,"b":10},
                    showlegend=True
                )
                st.plotly_chart(fig_ant,use_container_width=True)
            st.markdown('<div class="info-box"><b>Base stations</b> use 120° sector antennas (3-sector omnidirectional coverage). <b>Core routers</b> use narrow 2° pencil-beam microwave dishes for 13 GHz backbone. EIRP = TX power + antenna gain.</div>',unsafe_allow_html=True)
    ...
    with ant_tab2:
        ...
        if df_dim is not None:
            ...
            c1,c2,c3,c4=st.columns(4)
            with c1: st.metric("Channels / site",int(per_site["channels_required"].iloc[0]) if not per_site.empty else "-")
            with c2: st.metric("Worst blocking",f"{per_site['achieved_blocking'].max()*100:.3f}%" if not per_site.empty else "-")
            ...
            with c4:
                if not per_site.empty and not trunk.empty:
                    gain=per_site["channels_required"].sum()-int(trunk["channels_required"].iloc[0])
            ...
            st.markdown('<div class="info-box"><b>Erlang B:</b> B(A,N) = (A^N/N!) / Σ(A^k/k!) for k=0..N. Loop N upward until B ≤ 2%. <b>Trunking gain:</b> pooling 5 sites at CR-1 requires fewer trunk circuits vs 5×N individual circuits.</div>',unsafe_allow_html=True)
        else:
    ...
    with ant_tab3:
        ...
        if df_delay is not None:
            ...
            cls_badge={"telemetry":'<span class="badge-info">EF</span>',"video":'<span class="badge-warn">AF21</span>',"voice":'<span class="badge-pass">AF31</span>'}
...
# ─── QoS — independent voice/video/telemetry + failure order indicator ─────────
elif section=="QoS":
    st.markdown('<div class="section-header">QoS — Per-Class Analysis, Scheduler Design & Failure Order</div>',unsafe_allow_html=True)

    # Determine failure order from stress test
    failure_order=[]
    first_fail_class=None
    if df_stress is not None:
        for _,row in df_stress.iterrows():
            if not row.get("voice_kpi_met",True) and "Voice" not in failure_order:
                failure_order.append(("Voice",float(row["alpha"])))
            if not row.get("video_kpi_met",True) and "Video" not in failure_order:
                failure_order.append(("Video",float(row["alpha"])))
        if failure_order:
            failure_order.sort(key=lambda x:x[1])
            first_fail_class=failure_order[0][0]
    else:
        failure_order=[("Voice",1.5),("Video",2.0)]
        first_fail_class="Voice"

    if first_fail_class:
        order_str=" → ".join([f"<b>{cls}</b> (α={a:.1f})" for cls,a in failure_order])
        st.markdown(f'<div class="fail-first-banner">⚠ KPI Failure Order: {order_str} — <b>{first_fail_class}</b> is the first class to breach its KPI under load. Telemetry (EF strict priority) never fails.</div>',unsafe_allow_html=True)

    if df_delay is not None:
        qos_tab1,qos_tab2,qos_tab3,qos_tab4=st.tabs(["🎙 Voice","🎥 Video","📟 Telemetry","⚙ Scheduler Design"])

        def qos_class_view(cls_name,dscp_badge,target_ms,kpi_desc):
            cls_df=df_delay[df_delay["service_class"]==cls_name.lower()] if "service_class" in df_delay.columns else pd.DataFrame()
            if cls_df.empty:
                st.info(f"No {cls_name} data in delay KPI file.")
                return
            all_pass=bool(cls_df["met"].all())
            worst_site=cls_df.loc[cls_df["delay_ms"].idxmax(),"site"]
            worst_delay=cls_df["delay_ms"].max()
            is_first_fail=first_fail_class and cls_name==first_fail_class

            c1,c2,c3,c4=st.columns(4)
            with c1:
                badge_html='<span class="badge-first-fail">FIRST TO FAIL</span>' if is_first_fail and not all_pass else (f'<span class="badge-pass">ALL PASS</span>' if all_pass else '<span class="badge-fail">KPI BREACH</span>')
                st.markdown(f"**KPI Status** {badge_html}",unsafe_allow_html=True)
            with c2: st.metric("Worst P95 Delay",f"{worst_delay:.2f} ms",delta=f"Target ≤{target_ms} ms",delta_color="inverse" if worst_delay>target_ms else "normal")
            with c3: st.metric("Worst Site",worst_site)
            with c4: st.metric("Sites checked",len(cls_df))

            if is_first_fail and not all_pass:
                st.markdown(f'<div class="qos-fail-order">🔴 <b>{cls_name}</b> is the <b>first service class to breach its KPI</b> under increased load. This is the primary bottleneck — address channel dimensioning before video.</div>',unsafe_allow_html=True)

            rows="".join(f"<tr><td><b style='color:{PLT_PURPLE}'>{r.site}</b></td><td>{r.delay_ms:.2f} ms</td><td>{r.kpi_target} ms</td><td>{'<span class=\"badge-pass\">PASS</span>' if r.met else '<span class=\"badge-fail\">FAIL</span>'}</td><td>{'🟢' if r.met else '🔴'}</td></tr>" for r in cls_df.itertuples())
            st.markdown(f'<div class="table-wrap"><table class="styled-table"><thead><tr><th>Site</th><th>P95 Delay</th><th>Target</th><th>KPI</th><th></th></tr></thead><tbody>{rows}</tbody></table></div>',unsafe_allow_html=True)

            if PLOTLY_OK:
                colors_bar=[PLT_RED if not r.met else (PLT_AMBER if r.delay_ms>target_ms*0.8 else PLT_GREEN) for r in cls_df.itertuples()]
                fig_q=go.Figure()
                fig_q.add_trace(go.Bar(x=cls_df["site"].tolist(),y=cls_df["delay_ms"].tolist(),marker_color=colors_bar,name=f"{cls_name} P95"))
                fig_q.add_hline(y=target_ms,line_dash="dot",line_color=PLT_AMBER,annotation_text=f"Target {target_ms} ms",annotation_font_color=PLT_AMBER)
                fig_q.update_layout(height=260,paper_bgcolor=PLT_BG,plot_bgcolor=PLT_AX,font={"color":PLT_TEXT},xaxis_title="Site",yaxis_title="P95 Delay (ms)",showlegend=False,margin={"l":0,"r":0,"t":20,"b":0})
                st.plotly_chart(fig_q,use_container_width=True)
            st.markdown(f'<div class="info-box"><b>{cls_name} KPI:</b> {kpi_desc}</div>',unsafe_allow_html=True)

        with qos_tab1:
            qos_class_view("Voice",'<span class="badge-pass">AF31</span>',200,"P95 setup delay ≤ 200 ms. Voice uses Erlang B dimensioning — blocking ≤ 2%. First to fail under overload due to circuit-count bottleneck.")
        with qos_tab2:
            qos_class_view("Video",'<span class="badge-warn">AF21</span>',150,"P95 end-to-end delay ≤ 150 ms. Video is 40% WFQ allocation. Fails second after voice due to bandwidth saturation.")
        with qos_tab3:
            qos_class_view("Telemetry",'<span class="badge-info">EF</span>',50,"P95 delay ≤ 50 ms. Telemetry uses Expedited Forwarding (strict priority) — always served first. Never fails under any modeled load.")
        with qos_tab4:
            st.markdown('<div class="section-header">Scheduler Architecture</div>',unsafe_allow_html=True)
            col1,col2=st.columns(2)
            with col1:
                st.markdown('<div class="table-wrap"><table class="styled-table"><thead><tr><th>Class</th><th>DSCP</th><th>Scheduler</th><th>BW</th><th>KPI</th><th>Fail Order</th></tr></thead><tbody><tr><td>Telemetry</td><td><span class="badge-info">EF 46</span></td><td>Strict Priority</td><td>—</td><td>P95 ≤ 50 ms</td><td>Never fails</td></tr><tr><td>Voice</td><td><span class="badge-pass">AF31 26</span></td><td>WFQ 30%</td><td>30%</td><td>Block ≤ 2%</td><td><span class="badge-first-fail">1st</span></td></tr><tr><td>Video</td><td><span class="badge-warn">AF21 18</span></td><td>WFQ 40%</td><td>40%</td><td>P95 ≤ 150 ms</td><td><span class="badge-fail">2nd</span></td></tr><tr><td>Best effort</td><td>0</td><td>WFQ 30%</td><td>30%</td><td>—</td><td>Always last</td></tr></tbody></table></div>',unsafe_allow_html=True)
            with col2:
                st.markdown('<div class="info-box"><b>Why voice fails first:</b> Voice is circuit-switched (Erlang B). The bottleneck is N circuits, not bandwidth. Adding more bandwidth does not fix voice blocking — you must increase channel count.<br><br><b>Why telemetry never fails:</b> Expedited Forwarding preempts all other queues. As long as telemetry load is low (which it is — 0.02 Mbps), it always gets served within 50 ms.<br><br><b>Video fails second:</b> Video occupies 40% WFQ share. When primary load pushes above α=2×, the WFQ share is exhausted and P95 exceeds 150 ms.</div>',unsafe_allow_html=True)
    else:
        st.warning("QoS delay KPI data is currently unavailable.")

# ─── ROUTING & SIGNALING ──────────────────────────────────────────────────────
elif section=="Routing & Signaling":
    st.markdown('<div class="section-header">Routing Tables & Signaling Load Overview</div>',unsafe_allow_html=True)
    col1,col2=st.columns(2)
    with col1:
    ...
    with col2:
        ...
        if df_signal_summary is not None:
            ...
            bhca_val=summary.get('voice_busy_hour_attempts_hr',summary.get('total_bhca',0))
            delay_val=summary.get('worst_call_setup_delay_ms',summary.get('worst_setup_delay_ms',0))
            kpi_val=bool(summary.get('network_kpi_met',summary.get('all_kpis_met',False)))
    ...
    if df_signal is not None:
        ...
        target=float(df_signal['call_setup_target_ms'].iloc[0]) if 'call_setup_target_ms' in df_signal.columns else 50
        ax.bar(df_signal['site'].tolist(),df_signal['call_setup_delay_ms'].tolist(),color=[PLT_GREEN if d<=target else PLT_RED for d in df_signal['call_setup_delay_ms']],alpha=0.85,zorder=3,width=0.6)
        ax.axhline(target,color=PLT_AMBER,linestyle='--',linewidth=2,label=f'KPI Target ({target:.0f} ms)',zorder=4)
...
# ─── SIGNALING DETAILS ────────────────────────────────────────────────────────
elif section=="Signaling Details":
    st.markdown('<div class="section-header">SS7 / SIP Signaling Deep-Dive — Call Setup Message Flow</div>',unsafe_allow_html=True)
...
<div class="ss7-flow">
  ...
  <span class="ss7-label">│</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="ss7-label">│</span>&nbsp;◀─ RLC (Release Compl.) ─<span class="ss7-label">│</span>
        ...
        st.markdown('<div class="info-box"><b>Signaling model:</b> SS7 ISUP-inspired. IAM + ACM + ANM + REL + RLC = 5 core messages. Processing delay per CR node = 5 ms.</div>',unsafe_allow_html=True)
    with col_params:
        st.markdown("**Signaling Parameters**")
        SIG_MSG_BYTES=100;SIG_MSGS_PCALL=2;CHAN_CAP_KBPS=64.0;PROC_DELAY_MS=5.0
...
<tr><td>Utilisation target</td><td>&lt; 70%</td></tr>
    ...
    st.markdown('<div class="section-header">Normal vs Emergency Burst Signaling Analysis</div>',unsafe_allow_html=True)
    if scenario_obj:
    ...
    if df_signal is not None and "call_setup_delay_ms" in df_signal.columns:
        for _,r in df_signal.iterrows():
            prop_delays[r["site"]]=float(r["call_setup_delay_ms"])-PROC_DELAY_MS
    else:
        prop_delays={"BS1":8.0,"BS2":8.0,"BS3":9.0,"BS4":7.0,"BS5":9.0}
    burst_data=[]
    for scenario_type,cps in [("Normal",normal_cps),("Emergency Burst",burst_cps)]:
        for site in bs_sites:
            ...
            service_rate_cps=CHAN_CAP_KBPS*1000/(SIG_MSG_BYTES*8*SIG_MSGS_PCALL)
            rho=cps/service_rate_cps if service_rate_cps>0 else 0
            q_delay=(rho/(1-rho))*PROC_DELAY_MS if rho<1.0 else float('inf')
            total_delay=setup_delay+(q_delay if not math.isinf(q_delay) else 200.0)
            burst_data.append({"Scenario":scenario_type,"Site":site,"CPS":round(cps,4),"Sig Load (kbps)":round(sig_load_kbps,3),"Chan Util (%)":round(util_pct,2),"Prop Delay (ms)":round(prop,2),"Queue Delay (ms)":round(q_delay,3) if not math.isinf(q_delay) else "∞","Total Delay (ms)":round(total_delay,2),"Tel SLA ≤50ms":"✅" if total_delay<=50 else "❌","Voice SLA ≤200ms":"✅" if total_delay<=200 else "❌","Util <70%":"✅" if util_pct<70 else "❌"})
    ...
    with b1: st.metric("Normal CPS",f"{normal_cps:.4f}")
    with b2: st.metric("Burst CPS",f"{burst_cps:.4f}",delta="5×")
    with b3: st.metric("Normal Load (avg)",f"{normal_df['Sig Load (kbps)'].mean():.3f} kbps")
    with b4: st.metric("Burst Load (avg)",f"{burst_only['Sig Load (kbps)'].mean():.3f} kbps",delta_color="inverse")
    tab1,tab2=st.tabs(["Normal Scenario","Emergency Burst Scenario"])
    with tab1: st.dataframe(normal_df.drop(columns=["Scenario"]),use_container_width=True,hide_index=True)
    with tab2: st.dataframe(burst_only.drop(columns=["Scenario"]),use_container_width=True,hide_index=True)
    if PLOTLY_OK:
        fig_sig=go.Figure()
        fig_sig.add_trace(go.Bar(x=bs_sites,y=normal_df["Chan Util (%)"].tolist(),name="Normal",marker_color=PLT_BLUE))
        fig_sig.add_trace(go.Bar(x=bs_sites,y=burst_only["Chan Util (%)"].tolist(),name="Emergency Burst",marker_color=PLT_RED))
        fig_sig.add_hline(y=70,line_dash="dot",line_color=PLT_AMBER,annotation_text="70% threshold",annotation_font_color=PLT_AMBER)
        fig_sig.update_layout(height=300,barmode="group",paper_bgcolor=PLT_BG,plot_bgcolor=PLT_AX,font={"color":PLT_TEXT},xaxis_title="Base Station",yaxis_title="Channel Utilisation (%)",legend={"orientation":"h","y":-0.2},margin={"l":0,"r":0,"t":20,"b":0})
        st.plotly_chart(fig_sig,use_container_width=True)
    st.markdown('<div class="info-box"><b>M/M/1 queuing:</b> ρ = λ/μ. Queue delay = ρ/(1−ρ) × service_time. Channel well below 70% at normal CPS; emergency burst scales linearly.</div>',unsafe_allow_html=True)

# ─── CALL TERMINATION ─────────────────────────────────────────────────────────
elif section=="Call Termination":
    st.markdown('<div class="section-header">Call Termination & Lifecycle Management</div>',unsafe_allow_html=True)
  ...
  <b style="color:#534AB7">● IDLE</b>
    ...
    with col_stats:
        ...
        if scenario_obj:
            ...
            A_voice=voice_cfg.get("offered_load_erl",0.75);N_voice=4
        else:
            A_voice,N_voice=0.75,4
        def _eb(A,N):
            b=1.0
            for n in range(1,N+1): b=(A*b)/(n+A*b)
            return b
        block_prob=_eb(A_voice,N_voice)
        normal_term_pct=max(0,100-block_prob*100-2.1-1.4-0.8)
        reasons={"Normal user hang-up":round(normal_term_pct,1),"Blocked (no circuit)":round(block_prob*100,2),"Timeout (no answer)":2.1,"Network fault / reroute":1.4,"Radio link failure":0.8}
        if PLOTLY_OK:
            fig_pie=go.Figure(go.Pie(labels=list(reasons.keys()),values=list(reasons.values()),hole=0.45,marker_colors=[PLT_GREEN,PLT_RED,PLT_AMBER,PLT_BLUE,PLT_PURPLE],textfont_size=11))
            fig_pie.update_layout(height=280,paper_bgcolor=PLT_BG,font={"color":PLT_TEXT},legend={"font":{"size":11},"orientation":"v"},margin={"l":0,"r":0,"t":10,"b":0})
            st.plotly_chart(fig_pie,use_container_width=True)

    st.markdown('<div class="section-header">Sample Call Detail Records (CDR)</div>',unsafe_allow_html=True)
    st.markdown('<div class="info-box"><b>CDR fields:</b> Each completed call generates one CDR. Fields: calling/called number, start/end timestamp, duration, termination reason, DSCP class, site, charge units.</div>',unsafe_allow_html=True)
    rng=np.random.default_rng(42)
    sites_cdr=["BS1","BS2","BS3","BS4","BS5"]
    term_reasons_list=["Normal","Normal","Normal","Busy","No Answer","Fault"]
    ...
    for i in range(18):
        ...
        term=rng.choice(term_reasons_list,p=[0.75,0.10,0.08,0.04,0.02,0.01])
        charge=round(max(0,dur_s/60.0*(0.50 if svc=="voice" else 1.80 if svc=="video" else 0.05)),2)
        dscp={"voice":"AF31 (26)","video":"AF21 (18)","telemetry":"EF (46)"}[svc]
        cdr_rows.append({"CDR #":f"CDR-{2026042200+i:010d}","Site":site,"Service":svc.title(),"DSCP":dscp,"Start":start.strftime("%H:%M:%S"),"End":end.strftime("%H:%M:%S"),"Duration (s)":dur_s,"Termination":term,"Charge Units":charge})
    cdr_df=pd.DataFrame(cdr_rows)
    def cdr_row_html(r):
        tc=PLT_GREEN if r["Termination"]=="Normal" else PLT_RED if r["Termination"]=="Fault" else PLT_AMBER
        sc_map={"Voice":PLT_BLUE,"Video":PLT_PURPLE,"Telemetry":PLT_ACCENT}
    ...
    st.markdown(f'<div class="table-wrap"><table class="styled-table"><thead><tr><th>CDR #</th><th>Site</th><th>Service</th><th>DSCP</th><th>Start</th><th>End</th><th>Dur (s)</th><th>Termination</th><th>Charge</th></tr></thead><tbody>{rows_html}</tbody></table></div>',unsafe_allow_html=True)

# ─── BILLING ANALYTICS ────────────────────────────────────────────────────────
elif section=="Billing Analytics":
    st.markdown('<div class="section-header">Billing Analytics — Usage Metering & Revenue Model</div>',unsafe_allow_html=True)
    col_tariff,col_summary=st.columns([1,1.4],gap="large")
    with col_tariff:
        st.markdown("**Tariff Configuration**")
        voice_rate=st.number_input("Voice rate (BWP / min)",min_value=0.0,max_value=10.0,value=0.50,step=0.05)
        video_rate=st.number_input("Video rate (BWP / min)",min_value=0.0,max_value=20.0,value=1.80,step=0.10)
        tel_rate=st.number_input("Telemetry rate (BWP / min)",min_value=0.0,max_value=5.0,value=0.05,step=0.01)
        busy_hours=st.slider("Daily busy hours",1,16,8)
        days_month=st.slider("Days per month",20,31,22)
        st.markdown('<div class="info-box"><b>Currency:</b> BWP — Botswana Pula.</div>',unsafe_allow_html=True)

    if scenario_obj:
        tc=scenario_obj["traffic"]
        A_v=tc["voice"]["offered_load_erl"];h_v=tc["voice"]["holding_time_s"]/60.0
        A_vid=tc["video"]["offered_load_erl"];h_vid=min(tc["video"]["holding_time_s"],3600)/60.0
        A_tel=tc["telemetry"]["offered_load_erl"];h_tel=tc["telemetry"]["holding_time_s"]/60.0
        lam_v=tc["voice"]["arrival_rate_per_hour"];lam_vid=tc["video"]["arrival_rate_per_hour"];lam_tel=tc["telemetry"]["arrival_rate_per_hour"]
    else:
        A_v,h_v,lam_v=0.75,3.0,15.0;A_vid,h_vid,lam_vid=8.0,60.0,8.0;A_tel,h_tel,lam_tel=0.50,1.0,30.0
    ...
    st.markdown('<div class="section-header">Monthly Revenue Breakdown by Service & Site</div>',unsafe_allow_html=True)
    if PLOTLY_OK:
        site_labels=["BS1","BS2","BS3","BS4","BS5"]
        v_per_site=voice_rev_mo/n_sites;vid_per_site=video_rev_mo/n_sites;tel_per_site=tel_rev_mo/n_sites
        fig_bill=go.Figure()
        fig_bill.add_trace(go.Bar(x=site_labels,y=[v_per_site]*n_sites,name="Voice",marker_color=PLT_BLUE))
        fig_bill.add_trace(go.Bar(x=site_labels,y=[vid_per_site]*n_sites,name="Video",marker_color=PLT_PURPLE))
        fig_bill.add_trace(go.Bar(x=site_labels,y=[tel_per_site]*n_sites,name="Telemetry",marker_color=PLT_ACCENT))
        fig_bill.update_layout(height=320,barmode="stack",paper_bgcolor=PLT_BG,plot_bgcolor=PLT_AX,font={"color":PLT_TEXT},xaxis_title="Base Station",yaxis_title="Revenue (BWP/month)",legend={"orientation":"h","y":-0.25},margin={"l":0,"r":0,"t":20,"b":0})
        st.plotly_chart(fig_bill,use_container_width=True)

    st.markdown('<div class="section-header">Monthly Usage Volume & Charge Summary</div>',unsafe_allow_html=True)
    usage_data=[
        {"Service":"Voice","DSCP":"AF31","Arrivals/hr/site":lam_v,"Hold time (min)":round(h_v,1),"Monthly calls":f"{voice_calls_mo:,.0f}","Rate (BWP/min)":f"{voice_rate:.2f}","Monthly Rev (BWP)":f"{voice_rev_mo:,.2f}"},
        {"Service":"Video","DSCP":"AF21","Arrivals/hr/site":lam_vid,"Hold time (min)":round(h_vid,1),"Monthly calls":f"{video_calls_mo:,.0f}","Rate (BWP/min)":f"{video_rate:.2f}","Monthly Rev (BWP)":f"{video_rev_mo:,.2f}"},
        {"Service":"Telemetry","DSCP":"EF","Arrivals/hr/site":lam_tel,"Hold time (min)":round(h_tel,1),"Monthly calls":f"{tel_events_mo:,.0f}","Rate (BWP/min)":f"{tel_rate:.2f}","Monthly Rev (BWP)":f"{tel_rev_mo:,.2f}"},
    ]
    dscp_badges={"AF31":'<span class="badge-pass">AF31</span>',"AF21":'<span class="badge-warn">AF21</span>',"EF":'<span class="badge-info">EF</span>'}
    ...
    st.markdown(f'<div class="table-wrap"><table class="styled-table"><thead><tr><th>Service</th><th>DSCP</th><th>Arr/hr/site</th><th>Hold (min)</th><th>Monthly Calls</th><th>Rate (BWP/min)</th><th>Monthly Revenue</th></tr></thead><tbody>{rows_html}</tbody></table></div>',unsafe_allow_html=True)

    # ─── Export billing analytics ──────────────────────────────────────────────
    st.markdown('<div class="section-header">Export Billing Data</div>',unsafe_allow_html=True)
    export_col1,export_col2,export_col3=st.columns(3)
    usage_export_df=pd.DataFrame([
        {"Service":r["Service"],"DSCP":r["DSCP"],"Arrivals_per_hr_per_site":r["Arrivals/hr/site"],"Hold_time_min":r["Hold time (min)"],"Monthly_calls":r["Monthly calls"],"Rate_BWP_per_min":r["Rate (BWP/min)"],"Monthly_Revenue_BWP":r["Monthly Rev (BWP)"]} for r in usage_data
    ])
    usage_export_df.loc[len(usage_export_df)]={**{c:"" for c in usage_export_df.columns},"Service":"TOTAL","Monthly_Revenue_BWP":f"{total_rev_mo:,.2f}"}
    with export_col1:
        st.download_button("📥 Export Usage Summary (CSV)",data=usage_export_df.to_csv(index=False).encode("utf-8"),file_name="billing_usage_summary.csv",mime="text/csv",use_container_width=True)
    ...
    forecast_export_df=pd.DataFrame({"Year":years_fc,"Annual_Revenue_BWP":[round(r,2) for r in rev_fc],"Growth_Rate_pct":round(growth*100,1),"Voice_Monthly_BWP":round(voice_rev_mo,2),"Video_Monthly_BWP":round(video_rev_mo,2),"Telemetry_Monthly_BWP":round(tel_rev_mo,2),"Total_Monthly_BWP":round(total_rev_mo,2)})
    with export_col2:
        st.download_button("📥 Export Forecast (CSV)",data=forecast_export_df.to_csv(index=False).encode("utf-8"),file_name="billing_forecast.csv",mime="text/csv",use_container_width=True)
    # CDR export
    rng2=np.random.default_rng(42)
    sites_cdr2=["BS1","BS2","BS3","BS4","BS5"]
    term_reasons_list2=["Normal","Normal","Normal","Busy","No Answer","Fault"]
    now2=datetime(2026,4,22,8,0,0)
    cdr_rows2=[]
    for i in range(50):
        site=sites_cdr2[i%5];start=now2+timedelta(minutes=int(rng2.integers(0,480)));dur_s=int(rng2.exponential(180))+15;end=start+timedelta(seconds=dur_s)
        svc=rng2.choice(["voice","video","telemetry"],p=[0.55,0.35,0.10]);term=rng2.choice(term_reasons_list2,p=[0.75,0.10,0.08,0.04,0.02,0.01])
        charge=round(max(0,dur_s/60.0*(0.50 if svc=="voice" else 1.80 if svc=="video" else 0.05)),2)
        cdr_rows2.append({"CDR_ID":f"CDR-{2026042200+i:010d}","Site":site,"Service":svc,"DSCP":{"voice":"AF31","video":"AF21","telemetry":"EF"}[svc],"Start_Time":start.strftime("%Y-%m-%d %H:%M:%S"),"End_Time":end.strftime("%Y-%m-%d %H:%M:%S"),"Duration_s":dur_s,"Termination":term,"Charge_BWP":charge})
    cdr_export_df=pd.DataFrame(cdr_rows2)
    with export_col3:
        st.download_button("📥 Export CDR Records (CSV)",data=cdr_export_df.to_csv(index=False).encode("utf-8"),file_name="cdr_records.csv",mime="text/csv",use_container_width=True)

    st.markdown('<div class="section-header">5-Year Revenue Forecast (15% CAGR)</div>',unsafe_allow_html=True)
    if PLOTLY_OK:
        fig_fc=go.Figure()
        fig_fc.add_trace(go.Scatter(x=years_fc,y=rev_fc,mode="lines+markers",fill="tozeroy",fillcolor="rgba(218,119,86,0.12)",line={"color":PLT_ACCENT,"width":3},marker={"size":8,"color":PLT_ACCENT},name="Annual Revenue"))
        fig_fc.update_layout(height=300,paper_bgcolor=PLT_BG,plot_bgcolor=PLT_AX,font={"color":PLT_TEXT},xaxis_title="Year",yaxis_title="Revenue (BWP/year)",margin={"l":0,"r":0,"t":20,"b":0},showlegend=False)
        st.plotly_chart(fig_fc,use_container_width=True)
        c1,c2,c3=st.columns(3)
        with c1: st.metric("Year 0 (baseline)",f"BWP {rev_fc[0]:,.0f}")
        with c2: st.metric("Year 3 projection",f"BWP {rev_fc[3]:,.0f}",delta=f"+{(rev_fc[3]/rev_fc[0]-1)*100:.0f}%")
        with c3: st.metric("Year 5 projection",f"BWP {rev_fc[5]:,.0f}",delta=f"+{(rev_fc[5]/rev_fc[0]-1)*100:.0f}%")
    st.markdown(f'<div class="info-box"><b>Billing model:</b> CDR-based post-paid metering. Revenue = Σ(λ × h × rate × sites × hours × days). Growth at {growth*100:.0f}%/year CAGR. Currency: Botswana Pula (BWP).</div>',unsafe_allow_html=True)

# ─── STRESS TEST & FAULT MANAGEMENT (unified tab) ─────────────────────────────
elif section=="Stress Test & Fault Management":
    st.markdown('<div class="section-header">Stress Test, Fault Management & Breaking Point Analysis</div>',unsafe_allow_html=True)

    st_tab1,st_tab2=st.tabs(["📈 Stress Test & Breaking Point","🔧 Fault Management"])
    ...
    with st_tab1:
        if df_stress is not None:
            ...
            if not row.empty:
                ...
                if not r["voice_kpi_met"]: st.error(f"Voice blocking {r['voice_blocking']*100:.2f}% > 2% at α={r['alpha']}×. Fix: increase N channels per site.")
                else: st.success(f"All KPIs met at α={r['alpha']}×.")
            fig,ax=plt.subplots(figsize=(10,4))
            ax.plot(df_stress["alpha"],df_stress['voice_blocking']*100,'o-',color=PLT_ACCENT,lw=2.5,label='Voice Blocking (%)',ms=6)
            ax.axhline(2,color=PLT_AMBER,linestyle='--',linewidth=2,label='Target (2%)')
            ax.set_xlabel('Load Multiplier α');ax.set_ylabel('Blocking Probability (%)');ax.grid(axis='y');ax.legend()
            plt.tight_layout();st.pyplot(fig);plt.close(fig)
            rows=""
            ...
            st.markdown(f'<div class="table-wrap"><table class="styled-table"><thead><tr><th>α</th><th>Voice blocking</th><th>Video P95</th><th>Voice KPI</th></tr></thead><tbody>{rows}</tbody></table></div>',unsafe_allow_html=True)
        else:
            st.warning("Stress-test data is currently unavailable.")
        st.markdown('<div class="info-box"><b>Breaking point:</b> Voice fails first (blocking > 2%) — bottleneck is circuit count N, not bandwidth. Video KPI fails next as traffic saturates the 40% WFQ share. Telemetry (strict priority) never fails.</div>',unsafe_allow_html=True)

        # ─── Hidden: Custom Configuration (collapsible) ──────────────────────
        with st.expander("⚙ Custom Configuration (Advanced)", expanded=False):
            st.markdown('<div class="section-header">Network Configuration Parameters</div>',unsafe_allow_html=True)
            cfg=st.session_state.nms_config
            with st.form("config_form_stress"):
                routing_policy=st.selectbox("Routing policy",["Adaptive Failover","Load Balanced","Pinned Primary"],index=["Adaptive Failover","Load Balanced","Pinned Primary"].index(cfg["routing_policy"]))
                scheduler=st.selectbox("Scheduler",["Priority Queuing","Weighted Fair Queuing","Hybrid QoS"],index=["Priority Queuing","Weighted Fair Queuing","Hybrid QoS"].index(cfg["scheduler"]))
                col_cf1,col_cf2=st.columns(2)
                with col_cf1:
                    primary_bw=st.slider("Primary link BW (%)",50,130,int(cfg["primary_bw_alloc_pct"]))
                    backup_bw=st.slider("Backup link BW (%)",50,130,int(cfg["backup_bw_alloc_pct"]))
                    backbone_bw=st.slider("Backbone BW (%)",50,130,int(cfg["backbone_bw_alloc_pct"]))
                with col_cf2:
                    reuse_p=st.selectbox("Freq reuse pattern",["3/9","4/12","7/21"],index=["3/9","4/12","7/21"].index(cfg["reuse_pattern"]))
                    access_freq=st.number_input("Access frequency (MHz)",min_value=700,max_value=3800,value=int(cfg["access_frequency_mhz"]),step=100)
            ...
            if save_cfg:
                st.session_state.nms_config={**cfg,"routing_policy":routing_policy,"scheduler":scheduler,"primary_bw_alloc_pct":primary_bw,"backup_bw_alloc_pct":backup_bw,"backbone_bw_alloc_pct":backbone_bw,"reuse_pattern":reuse_p,"access_frequency_mhz":access_freq,"antenna_gain_dbi":ant_gain,"antenna_tilt_deg":ant_tilt,"antenna_height_adjust_m":ant_height,"auto_reroute":auto_r,"load_balancing":load_b}
            ...
            st.markdown(f'<div class="info-box"><b>Current:</b> Primary {cfg["primary_bw_alloc_pct"]}% | Backup {cfg["backup_bw_alloc_pct"]}% | Backbone {cfg["backbone_bw_alloc_pct"]}%<br>Access {cfg["access_frequency_mhz"]} MHz | Gain {cfg["antenna_gain_dbi"]} dBi | Tilt {cfg["antenna_tilt_deg"]}°</div>',unsafe_allow_html=True)

    with st_tab2:
        st.markdown('<div class="section-header">Active Fault Scenario</div>',unsafe_allow_html=True)
        ...
        with left:
            ...
            if apply_f:
                st.session_state.nms_faults={"backhaul_degradation_pct":backhaul_deg,"failed_bs":failed_bs,"router_congestion_router":cong_router,"router_congestion_pct":cong_pct}
            ...
            if reroute_check:
                ...
                for bs,info in reroute_check.get("bs_routes",{}).items():
                    if info.get("reachable"): st.success(f"✓ {bs} → CR-2: {len(info['path'])-1} hops")
                    else: st.error(f"✗ {bs} → NO PATH")
        with right:
            ...
            if alarms_df.empty:
                ...
                rows="".join(f"<tr><td>{r.entity}</td><td>{r.alarm}</td><td>{severity_badge(r.severity)}</td><td>{r.hint}</td></tr>" for r in alarms_df.itertuples())
...
# ─── FORECAST ─────────────────────────────────────────────────────────────────
    ...
    st.markdown('<div class="section-header">5-Year Capacity Forecast</div>',unsafe_allow_html=True)
    if df_util is None:
        ...
        try:
            ...
            if os.path.exists(fc_path):
                with st.spinner("Running forecasting.py..."):
                    result=subprocess.run([sys.executable,fc_path],cwd=BASE,capture_output=True,timeout=60)
                df_util=load_csv("forecasting_utilisation_annual.csv")
                df_plan=normalize_upgrade_plan_df(load_csv("forecasting_upgrade_plan.csv"))
                if df_util is not None:
                    st.success("Forecasting module ran successfully.")
                else:
                    st.warning("Forecasting ran but no output CSV found. Check forecasting.py outputs.")
    ...
    if df_util is not None:
        ...
        with col2:
            if df_plan is not None:
                ...
                trigger_col="Trigger" if "Trigger" in df_plan.columns else (df_plan.columns[1] if len(df_plan.columns)>1 else phase_col)
                action_col="Action" if "Action" in df_plan.columns else (df_plan.columns[2] if len(df_plan.columns)>2 else phase_col)
                goal_col="Goal" if "Goal" in df_plan.columns else (df_plan.columns[3] if len(df_plan.columns)>3 else "")
        ...
        if PLOTLY_OK:
            fig_fc2=go.Figure()
            fig_fc2.add_trace(go.Scatter(x=df_util["year"].tolist(),y=(df_util["utilisation"]*100).tolist(),mode="lines+markers",line={"color":PLT_ACCENT,"width":3},marker={"size":7},name="Utilisation %",fill="tozeroy",fillcolor="rgba(218,119,86,0.10)"))
            fig_fc2.add_hline(y=70,line_dash="dot",line_color=PLT_AMBER,annotation_text="Planning threshold 70%")
            fig_fc2.add_hline(y=90,line_dash="dot",line_color=PLT_RED,annotation_text="Action threshold 90%")
            fig_fc2.update_layout(height=280,paper_bgcolor=PLT_BG,plot_bgcolor=PLT_AX,font={"color":PLT_TEXT},xaxis_title="Year",yaxis_title="Utilisation (%)",margin={"l":0,"r":0,"t":20,"b":0},showlegend=False)
